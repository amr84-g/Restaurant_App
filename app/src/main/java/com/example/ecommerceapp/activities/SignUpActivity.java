...
public class SignUpActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    private void AddProducts() {
        Category pizza = null;
        Category hamburger = null;
        Category pasta = null;
        try {
            pizza = new GetCategoryID(RoomFactory.getNotesDatabase(SignUpActivity.this).getCustomerDAO()).execute("pizza").get();
            hamburger = new GetCategoryID(RoomFactory.getNotesDatabase(SignUpActivity.this).getCustomerDAO()).execute("hamburger").get();
            pasta = new GetCategoryID(RoomFactory.getNotesDatabase(SignUpActivity.this).getCustomerDAO()).execute("pasta").get();
        ...
        AddpizzaProduct(pizza);
        AddTvProducts(pasta);
    }

    private void AddpizzaProduct(Category pizza) {

        for (int i = 1; i < 5; i++) {
            Product p1 = new Product();
            p1.setCat_ID(pizza.getCatID());
            p1.setPrice((5000*i)/2);
            p1.setProName("test ");
            p1.setQuantity(5+i);
            p1.setPro_img(String.valueOf(R.drawable.test));
            new insertProduct(RoomFactory.getNotesDatabase(SignUpActivity.this).getCustomerDAO()).execute(p1);

            Product p2 = new Product();
            p2.setCat_ID(pizza.getCatID());
            p2.setPrice(3000*i);
            p2.setProName("marg");
            p2.setQuantity(5+i);
            p2.setPro_img(String.valueOf(R.drawable.marg));
            new insertProduct(RoomFactory.getNotesDatabase(SignUpActivity.this).getCustomerDAO()).execute(p2);

            Product p3 = new Product();
            p3.setCat_ID(pizza.getCatID());
            p3.setPrice(5000);
            p3.setProName("salame ");
            p3.setQuantity(5);
            p3.setPro_img(String.valueOf(R.drawable.salme));
    ...
    private void AddTvProducts(Category tv) {
        for (int i=1 ;i<8;i++) {
            ...
            p1.setProName("tomato-salt");
            p1.setQuantity((5+i));
            p1.setPro_img((String.valueOf(R.drawable.pasta)));
            new insertProduct(RoomFactory.getNotesDatabase(SignUpActivity.this).getCustomerDAO()).execute(p1);
        }
    }
